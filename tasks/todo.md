...
> **Status (2026-07-18): all 16 tasks below are DONE.** The product has since
> moved well past this original single-mandal plan — it is now multi-tenant
> (a `mandals` table + `mandal_id` on every row), branding uploads go to
> Cloudinary (not Supabase Storage), the transparency chart is a hand-rolled
> inline-SVG donut (not Recharts), and the 2026-07-18 audit waves (money
> integrity, people flows, resilience, polish) are implemented on top. Treat
> the task text below as historical; `SPEC.md` + the code are the source of
> truth. Boxes are checked to reflect completion, not the original wording.
...
- [x] **Task 2: Supabase schema + RLS + seed**
...
- [x] **Task 3: Money + reconciliation core**
...
- [x] **Task 4: Admin auth (email magic link)**
...
- [x] **Task 5: Volunteer invite links**
...
- [x] **Task 6: Settings / mandal config**
...
- [x] **Task 7: Collection form → donation row**
...
- [x] **Task 15: Master ledger + books-balance light**
...
- [x] **Task 16: Transparency report (public)**
