    ...
    <div className="min-h-screen bg-stone-50 font-body">
      <main className="mx-auto flex max-w-2xl flex-col gap-5 px-4 py-8">
        <div className="flex items-center justify-between gap-3">
          <h1 className="font-display text-2xl font-extrabold tracking-tight text-stone-900">{t.title}</h1>
          {removed.length > 0 && (
            <button
              type="button"
              onClick={() => setShowRemoved((s) => !s)}
              className="rounded-lg px-2.5 py-1.5 text-sm font-semibold text-stone-500 hover:bg-stone-100"
            >
            ...
            {visible.map((donation) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span
                    className={`flex-none text-lg font-bold ${donation.voided ? 'text-stone-400 line-through' : 'text-stone-900'}`}
                  >
        ...
        {isAdmin && active.length > 0 && (
          <section className="mt-4 rounded-2xl border border-red-200 bg-red-50/50 p-5">
        ...
        <div>
          <Link to={isAdmin ? '/admin' : '/volunteer'} className="text-sm font-semibold text-orange-600 hover:text-orange-700">
            ← {isAdmin ? strings.admin.dashboardTitle : strings.collection.title}
          </Link>
        </div>
      </main>
